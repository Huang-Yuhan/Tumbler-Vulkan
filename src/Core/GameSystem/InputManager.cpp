...
static int TranslateToGLFWMouseButton(EKeyCode code) {
    switch (code) {
        case EKeyCode::MouseLeft:
            return GLFW_MOUSE_BUTTON_LEFT;
        case EKeyCode::MouseRight:
            return GLFW_MOUSE_BUTTON_RIGHT;
        case EKeyCode::MouseMiddle:
...
void InputManager::Tick() {
    ...
    bool bMouseRightJustPressed = CurrentKeys[static_cast<uint16_t>(EKeyCode::MouseRight)] &&
                                  !PreviousKeys[static_cast<uint16_t>(EKeyCode::MouseRight)];
    bool bMouseRightJustReleased = !CurrentKeys[static_cast<uint16_t>(EKeyCode::MouseRight)] &&
                                   PreviousKeys[static_cast<uint16_t>(EKeyCode::MouseRight)];
...
float InputManager::GetAxis(const std::string& name) const {
    if (IsInputBlocked())
        return 0.0f; // 如果在输入 UI，禁止移动角色

    auto it = AxisBindings.find(name);
    if (it != AxisBindings.end()) {
        float val = 0.0f;
        if (CurrentKeys[static_cast<uint16_t>(it->second.PosKey)])
            val += 1.0f;
        if (CurrentKeys[static_cast<uint16_t>(it->second.NegKey)])
            val -= 1.0f;
        return val;
    }
    return 0.0f;
...
bool InputManager::IsActionJustPressed(const std::string& name) const {
    ...
    if (it != ActionBindings.end()) {
        ...
        return CurrentKeys[static_cast<uint16_t>(key)] &&
               !PreviousKeys[static_cast<uint16_t>(key)]; // 只有当前按下且上一帧没按，才是“刚刚按下”
    }
    return false;
}
