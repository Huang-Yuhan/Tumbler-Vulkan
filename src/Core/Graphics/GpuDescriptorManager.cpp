...
bool GpuDescriptorManager::Init(VkDevice device, uint32_t maxTextures) {
    m_Device = device;
    ...
    LOG_INFO("GpuDescriptorManager initialized (max textures: {})", maxTextures);
    return true;
}

void GpuDescriptorManager::Shutdown() {
    if (m_Set1Layout) {
...
VkDescriptorSet GpuDescriptorManager::AllocateSet(VkDescriptorSetLayout layout) {
...
void GpuDescriptorManager::FreeSet(VkDescriptorSet set) {
    vkFreeDescriptorSets(m_Device, m_Pool, 1, &set);
}

void GpuDescriptorManager::UpdateSet0(VkDescriptorSet set, VkBuffer sceneUBO, VkImageView shadowMapView,
...
void GpuDescriptorManager::UpdateSet1Bindless(VkDescriptorSet set, const std::vector<VkImageView>& textureViews,
