...
bool RenderDevice::Init(VkDevice device, VkPhysicalDevice physicalDevice) {
    ...
    LOG_INFO("RenderDevice initialized (VMA)");
    return true;
}
...
BufferHandle RenderDevice::CreateBuffer(size_t size, VkBufferUsageFlags usage, VmaMemoryUsage memory) {
...
BufferHandle RenderDevice::CreateBuffer(const VkBufferCreateInfo& ci, const VmaAllocationCreateInfo& ai,
...
ImageHandle RenderDevice::CreateImage(const VkImageCreateInfo& ci, const VmaAllocationCreateInfo& ai) {
    ImageHandle handle;
...
VkSampler RenderDevice::CreateSampler(VkFilter min, VkFilter mag, VkSamplerAddressMode addressMode, uint32_t maxLod) {
