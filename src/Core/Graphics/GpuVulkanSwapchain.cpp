...
bool GpuSwapchain::Init(VkInstance instance, VkPhysicalDevice physicalDevice, VkDevice device, VkSurfaceKHR surface,
                           uint32_t graphicsQueueFamily, uint32_t presentQueueFamily, GpuDevice& renderDevice,
    ...
    LOG_INFO("GpuSwapchain initialized ({}x{})", width, height);
    return true;
}

void GpuSwapchain::Shutdown() {
    DestroySwapchainObjects();
    LOG_INFO("GpuSwapchain shutdown");
}

void GpuSwapchain::Recreate(int width, int height) {
    DestroySwapchainObjects();
...
void GpuSwapchain::DestroySwapchainObjects() {
    if (m_DepthImageView) {
...
VkResult GpuSwapchain::AcquireNextImage(uint32_t* imageIndex, VkSemaphore semaphore, VkFence fence) {
...
VkResult GpuSwapchain::Present(VkQueue queue, uint32_t imageIndex, VkSemaphore waitSemaphore) {
