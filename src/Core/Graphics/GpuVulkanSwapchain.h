...
public:
    bool Init(VkInstance instance, VkPhysicalDevice physicalDevice, VkDevice device, VkSurfaceKHR surface,
              uint32_t graphicsQueueFamily, uint32_t presentQueueFamily, GpuDevice& renderDevice, int width,
              int height);
    void Shutdown();
...
private:
    ...
    GpuDevice* m_RenderDevice = nullptr;
